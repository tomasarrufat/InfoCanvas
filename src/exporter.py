...
"}",
"function infoRectVisible(elem){",
"  return elem && (elem.dataset.showOnHover==='false' || elem.style.opacity!=='0');",
"}",
"function updateLineVisibility(){",
"  document.querySelectorAll('.connection-line').forEach(function(svg){",
"    var s=document.querySelector('.info-rectangle-export[data-id="'+svg.dataset.source+'"]');",
"    var d=document.querySelector('.info-rectangle-export[data-id="'+svg.dataset.destination+'"]');",
"    svg.style.display= (infoRectVisible(s)||infoRectVisible(d))?'block':'none';",
"  });",
"}",
"document.querySelectorAll('.hotspot.info-rectangle-export').forEach(function(h){",
"  if(h.dataset.showOnHover!=='false'){",
"    h.addEventListener('mouseenter',function(){h.style.opacity='1'; updateLineVisibility(); updateConnectionLines();});",
"    h.addEventListener('mouseleave',function(){h.style.opacity='0'; updateLineVisibility(); updateConnectionLines();});",
"  }",
